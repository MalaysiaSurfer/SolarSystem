...
class App:
    ...
    def create_main_root(self) -> None:
        ...
        self.root_elements["credits"] = tk.Label(
            self.root,
            text=data.Credits.text,
            font=data.Credits.font,
            bg=data.Credits.bg_color,
            fg=data.Credits.text_color,
        )

        data.Controls.font = tkFont.Font(family="Helvetica", size=12, weight="bold")
        self.root_elements["controls"] = tk.Label(
            self.root,
            text=data.Controls.text,
            font=data.Controls.font,
            bg=data.Controls.bg_color,
            fg=data.Controls.text_color,
        )

        data.Time_text.font = tkFont.Font(family="Magneto", size=48, weight="bold")
        data.Time_text.text = f"{main.format_ymwd(time_past)}"
        self.root_elements["time_text"] = tk.Label(
            self.root,
            text=data.Time_text.text,
            font=data.Time_text.font,
            bg=data.Time_text.bg_color,
            fg=data.Time_text.text_color,
        )
        ...
        self.root_elements["trails_button"] = tk.Button(
            self.root, text="Hide Trails", command=self.toggle_trails
        )
        self.root_elements["stars_button"] = tk.Button(
            self.root, text="Hide Stars", command=self.hide_stars
        )
        self.root_elements["moons_button"] = tk.Button(
            self.root, text="Hide Moons", command=self.hide_moons
        )
        self.root_elements["IsBH"] = tk.Button(
            self.root, text="Spawn Black Hole", command=self.spawn
        )
        self.root_elements["BHHighlight"] = tk.Button(
            self.root, text="Highlight Black Hole", command=self.BHHighlight
        )
        self.root_elements["text_button"] = tk.Button(
            self.root, text="Hide Text", command=self.hide_text
        )
        if not blackhole and not kepler_check:
            ...
            planet_names = [
                body.name for body in data.bodies if body.name != "BlackHole"
            ]
            self.root_elements["planet_combobox"] = ttk.Combobox(
                self.root,
            ...
            self.root_elements["planet_combobox"].bind(
                "<<ComboboxSelected>>", self.on_planet_select
            )

        self.multiplier_var = tk.DoubleVar(value=1.0)
        self.root_elements["planet_scale"] = tk.Scale(
            self.root,
            from_=0.1,
            to=10.0,
            resolution=0.1,
        ...
        self.root_elements["planet_scale_text"] = tk.Label(
            self.root, text="Mass Multiplier: 1.0", fg="white", bg="#000000"
        )
        ...
        self.root_elements["credits"].place(
            ...
            - self.root_elements["credits"].winfo_reqwidth(),
            y=self.root.winfo_screenheight()
            - self.root_elements["credits"].winfo_reqheight() * 2,
        )
        self.root_elements["controls"].place(
            x=0,
            y=self.root.winfo_screenheight()
            - self.root_elements["controls"].winfo_reqheight() * 1.5,
        )
        self.root_elements["time_text"].place(
            x=(
                self.root.winfo_screenwidth()
                - data.Time_text.font.measure(data.Time_text.text)
            )
            / 2,
        ...
        self.root_elements["trails_button"].place(
            x=self.root.winfo_screenwidth()
            - self.root_elements["trails_button"].winfo_reqwidth(),
            y=0,
        )
        self.root_elements["stars_button"].place(
            x=self.root.winfo_screenwidth()
            - self.root_elements["stars_button"].winfo_reqwidth(),
            y=30,
        )
        if not kepler_check and len(data.bodies) > 5:
            self.root_elements["moons_button"].place(
                x=self.root.winfo_screenwidth()
                - self.root_elements["moons_button"].winfo_reqwidth(),
                y=60,
            )
        if data.constants["user_scale"]:
            if center_is_jupiter or len(data.bodies) == 3:
                self.root_elements["text_button"].place(
                    x=self.root.winfo_screenwidth()
                    - self.root_elements["text_button"].winfo_reqwidth(),
                    y=60,
                )
            else:
                self.root_elements["text_button"].place(
        ...
        if (
            not data.constants["user_scale"]
            and not kepler_check
            and not center_is_jupiter
        ):
            if blackhole:
                self.root_elements["IsBH"].place(
                    x=self.root.winfo_screenwidth()
                    - self.root_elements["IsBH"].winfo_reqwidth(),
                    y=90,
                )
            else:
        ...
        if blackhole:
            self.root_elements["BHHighlight"].place(
                x=self.root.winfo_screenwidth()
                - self.root_elements["BHHighlight"].winfo_reqwidth(),
                y=120,
            )
            ...
            if data.constants["user_scale"]:
                if center_is_jupiter or len(data.bodies) == 3:
                    self.root_elements["planet_combobox"].place(
                        x=self.root.winfo_screenwidth()
                        - self.root_elements["planet_combobox"].winfo_reqwidth(),
                        y=90,
                    )
                else:
                    self.root_elements["planet_combobox"].place(
    ...
    def create_pre_root(self) -> None:
        if not self.root:
            ...
            btn4 = tk.Button(
                self.root,
                text="Solar System with Black Hole",
                font=font,
                command=self.black_hole,
                bg="#eae6ca",
            )
            btn5 = tk.Button(
                self.root,
            ...
            btn6 = tk.Button(
                self.root, text=" Realistic ", font=font, command=self.sem, bg="#eae6ca"
            )
            btn8 = tk.Button(
                self.root, text="Simplified", font=font, command=self.jm_s, bg="#eae6ca"
            )
            btn7 = tk.Button(
                self.root, text=" Realistic ", font=font, command=self.jm, bg="#eae6ca"
            )
            self.root.update_idletasks()

            text1.place(
                x=(self.root.winfo_screenwidth() - font.measure("Choose mode")) / 2,
                y=100,
            )
            text2.place(
                x=(self.root.winfo_screenwidth() - font.measure("Sun Earth Moon")) / 2
                - 265,
                y=650,
            )
            text3.place(
                x=(self.root.winfo_screenwidth() - font.measure("Jupiter and moons"))
                / 2
                + 300,
                y=650,
            )
            btn1.place(
                x=(self.root.winfo_screenwidth() / 2 - btn1.winfo_reqwidth() - 100),
                y=300,
            )
            btn2.place(x=(self.root.winfo_screenwidth() / 2 + 100), y=300)
            btn3.place(
                x=(self.root.winfo_screenwidth() / 2 - btn3.winfo_reqwidth() - 200),
                y=500,
            )
    ...
    def update(self) -> None:
        ...
        if not kepler_check:
            ...
            if "time_text" in self.root_elements and visible:
                self.root_elements["time_text"].config(
                    text=f"{main.format_ymwd(time_past)}"
                )
            if running:
            ...
            for body in data.bodies:
                if show_trails and len(body.trail) > 1:
                    self.canvas.create_line(
                        body.trail, fill=body.color, width=1, smooth=True
                    )
            ...
            if data.constants["user_scale"]:
                for body in data.bodies:
                    ...
                    if body.name == "Moon":
                        add_coord = data.Point(
                            data.Earth.screen_x - body.screen_x,
                            data.Earth.screen_y - body.screen_y,
                        )
                    ...
                    if body.name in data.jupiter_moons:
                        add_coord = data.Point(
                            data.Jupiter.screen_x - body.screen_x,
                            data.Jupiter.screen_y - body.screen_y,
                        )
                        match body.name:
                            case "Io":
                                scale = 18
                    ...
                    if (
                        body.name == "Moon" or body.name in data.jupiter_moons
                    ) and not show_moons:
                        continue
                    ...
                    planet_id = self.canvas.create_oval(
                        ...
                        - body.screen_radius * data.constants["scale_m"],
                        body.screen_y
                    ...
                    if body.name == "Saturn":
                        self.canvas.create_oval(
                            body.screen_x
                            + data.constants["move_mx"]
                            - (body.screen_radius + 10) * data.constants["scale_m"],
                            body.screen_y
                            + data.constants["move_my"]
                            - (body.screen_radius + 10) * data.constants["scale_m"],
                            body.screen_x
                            ...
                            + (body.screen_radius + 10) * data.constants["scale_m"],
                            outline="#ceb8b8",
                            width=6 * data.constants["scale_m"],
                        )

                    self.bind_planet_tooltip(
                        planet_id,
                        body.name,
                        body.mass,
                        body.radius,
                        math.hypot(body.Orbital_speed.x, body.Orbital_speed.y),
                        body.screen_x,
                        body.screen_y,
                    )
                    if show_text:
                        self.canvas.create_text(
                            body.screen_x + data.constants["move_mx"],
                            body.screen_y
                            + data.constants["move_my"]
                            - body.screen_radius * data.constants["scale_m"]
                            - 10,
                ...
                for body in data.bodies:
                    ...
                    if (
                        body.name == "Moon"
                        or body.name in data.jupiter_moons
                        and not show_moons
                    ):
                        continue
        ...
        if data.constants["user_scale"]:
            for i in self.stars:
                p = self.canvas.create_oval(
                    i[0], i[1], i[0] + 2, i[1] + 2, fill="white", outline="white"
                )
    ...
    def sem(self, t=False) -> None:
        ...
        data.bodies = [data.Sun, data.Earth, data.Moon]
        return self.open_main_root()
    ...
    def jm(self, t=False) -> None:
        ...
        data.bodies = [data.Jupiter, data.Io, data.Europa, data.Ganymede, data.Callisto]
        for body in data.bodies:
            match body.name:
                case "Jupiter":
                ...
                case "Io":
                    body.position = data.Point(
                        data.Jupiter.position.x, data.Jupiter.position.y + 4.217e8
                    )
        ...
        if pause_times >= 5 and not joker:
            ...
            threading.Thread(
                target=lambda: webbrowser.open(
                    "https://youtu.be/dQw4w9WgXcQ?si=EScuTA7EVvjQaglx"
                ),
                daemon=True,
            ).start()
    ...
    def bind_planet_tooltip(
        ...
        def show_tooltip(event) -> None:
            ...
            self.tooltip_text = tk.Label(
                self.tooltip, background="black", fg="white", font=("Helvetica", 10)
            )
